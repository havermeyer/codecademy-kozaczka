...
// scoop_db.yml is the database file. If that file doesn't not exist or fails
// to load, the default database object will be used.
...
function deleteComment(url, request) {
  ...
  if (savedComment) {
    ...
    //does changing the variables also change the database????
    ...
    //error in test code? should be 400
    response.status = 404;
  }
...
function upvoteComment(url, request) {
  ...
  if (request.body) {
    user = database.users[request.body.username];
    username = request.body.username;
    comment = database.comments[id];  
  }
  
  const response = {};

  if (comment && user) {
    const upvotedBy = database.comments[id].upvotedBy;
    const downvotedBy = database.comments[id].downvotedBy;
    //console.log(upvotedBy.indexOf(username));
    //console.log(username);
    if (upvotedBy.indexOf(username) === -1) {
      upvotedBy.push(username);
      //console.log(upvotedBy);
      let index = downvotedBy.indexOf(username);
      if (index >= 0) {
...
function downvoteComment(url, request) {
  ...
  if (comment && user) {
    ...
    if (downvotedBy.indexOf(username) === -1) {
      downvotedBy.push(username);
      
      let index = upvotedBy.indexOf(username);
      if (index >= 0) {
...
function loadDatabase() {
  var doc;
  try {
    doc = yaml.safeLoad(fs.readFileSync('scoop_db.yml', 'utf8'));
  } catch (e) {
    console.log(e);
...
function saveDatabase() {
  let t = yaml.safeDump(database);
  console.log(t);
  fs.writeFile('scoop_db.yml', t, (err) => {
